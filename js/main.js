...

{
  ...
  function createColumn(col){
    ...
    const column = [];
    // b[0] = source.splice(Math.floor(Math.random() * source.length), 1)[0];//splice=配列から切り出し
    // b[1] = source.splice(Math.floor(Math.random() * source.length), 1)[0];// [0]がないと、それぞれが配列になってしまう
